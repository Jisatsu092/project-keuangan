...
function journalForm() {
    return {
        ...
        onAccountChange(index) {
            // Auto-clear jika user memilih akun yang sama
            const currentAccount = this.rows[index].account_code;
            const duplicateIndex = this.rows.findIndex((row, i) => i !== index && row.account_code === currentAccount);
            
            if (duplicateIndex !== -1) {
                if (confirm('Akun ini sudah dipilih di baris lain. Apakah Anda yakin?')) {
                    return;
                } else {
                    this.rows[index].account_code = '';
                }
            }
        }
    }
