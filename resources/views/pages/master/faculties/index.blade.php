...
@section('content')
<div class="px-4 sm:px-0" x-data="{ showForm: false, editMode: false, editId: null, formData: { code: '', name: '', is_active: true } }">
    
    <!-- Header -->
    <div class="flex justify-between items-center mb-6">
        <div>
            <h2 class="text-2xl font-bold text-gray-900">Master Data - Fakultas</h2>
            <p class="mt-1 text-sm text-gray-600">Kelola data fakultas (Digit 3)</p>
        </div>
        <button @click="showForm = !showForm; editMode = false; formData = { code: '', name: '', is_active: true }" 
    ...
    <div x-show="showForm" x-collapse class="bg-white rounded-lg shadow-lg p-6 mb-6">
        <h3 class="text-lg font-semibold text-gray-900 mb-4" x-text="editMode ? 'Edit Fakultas' : 'Tambah Fakultas Baru'"></h3>
        
        <form :action="editMode ? '/master/faculties/' + editId : '{{ route('master.faculties.store') }}'" method="POST">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    <p class="mt-1 text-xs text-gray-500">
                        <i class="fas fa-info-circle"></i> Kode 1-9 (0 reserved untuk Pusat)
                    </p>
                </div>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-50">
                    <td class="px-6 py-4 whitespace-nowrap">
                        <span class="text-2xl font-bold font-mono text-blue-600">{{ $faculty->code }}</span>
                    </td>
                    <td class="px-6 py-4">
                        <span class="text-sm font-semibold text-gray-900">{{ $faculty->name }}</span>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                        <span class="px-2 py-1 bg-blue-100 text-blue-800 rounded text-sm">
                            {{ $faculty->prodis()->count() }} Prodi
                        </span>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <button @click="editMode = true; showForm = true; editId = {{ $faculty->id }}; formData = { code: '{{ $faculty->code }}', name: '{{ $faculty->name }}', is_active: {{ $faculty->is_active ? 'true' : 'false' }} }"
                ...
                <tr>
                    <td colspan="5" class="px-6 py-12 text-center">
                        ...
                        <p class="text-gray-500">Belum ada data fakultas</p>
                    </td>
                </tr>
                @endforelse
            </tbody>
